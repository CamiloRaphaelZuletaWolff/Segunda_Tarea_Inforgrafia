...
func _on_player_health_changed(new_health):
	player_health_label.text = "HP: %d" % new_health
...
func _update_labels():
	player_health_label.text = "HP: %d" % player.health
	enemya_health_label.text = "HP Enemy: %d" % enemya.health
	if gem_timer.is_stopped():
		gem_timer_label.text = "Gem Time: 0.0s"
	else:
		gem_timer_label.text = "Gem Time: %.1fs" % gem_timer.time_left
